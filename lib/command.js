...
define(['./compiler', 'optimist', 'fs'], function(compiler, optimist, fs) {
  ...
  main = function() {
    var a, aplCode, argv, b, cs, ctx, isCoffeeScript, jsCode, k, knownOptions, pp;
    ...
    if (argv.compile) {
      ...
      if (argv.stdio || argv.print) {
        ...
        return fs.writeFileSync(argv._[0].replace(/\.(apl|coffee)$/, '') + '.js', jsCode, 'utf8');
      }
    } else {
      return (new Function("var _ = arguments[0];\n" + jsCode))(createGlobalContext());
    }
  };
  repl = function(ctx) {
