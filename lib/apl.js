(function() {
  var BTM, BTMLFT, BTMRGT, LFT, PI, RGT, TOP, TOPLFT, TOPRGT, abs, acos, ambivalent, arrayValueOf, asin, atan, booleanValueOf, builtins, catenate, ceil, compressOrReplicate, cos, depthOf, dyadic, exec, exp, floor, format, format0, hpad, infixOperator, inherit, isSimple, log, main, match, max, min, monadic, niladic, numericValueOf, outerProduct, pervasive, postfixOperator, pow, prefixOperator, prod, random, reduce, repeat, reverse, shapeOf, sin, sqrt, sum, tan, vpad, withShape, _ref;
  ...
  monadic('⌽', reverse = function(a, _, axis) {
    ...
    r = [];
    for (i = 0; 0 <= ni ? i < ni : i > ni; 0 <= ni ? i++ : i--) {
      for (j = _ref2 = nj - 1; _ref2 <= 0 ? j <= 0 : j >= 0; j += -1) {
        for (k = 0; 0 <= nk ? k < nk : k > nk; 0 <= nk ? k++ : k--) {
  ...
  dyadic('↓', function(a, b) {
    ...
    lims = (function() {
      ...
      for (i = 0, _ref4 = a.length; 0 <= _ref4 ? i < _ref4 : i > _ref4; 0 <= _ref4 ? i++ : i--) {
        _results.push(a[i] >= 0 ? [min(a[i], sb[i]), sb[i]] : [0, max(0, sb[i] + a[i])]);
      }
      return _results;
    })();
    r = [];
  ...
  compressOrReplicate = function(a, b, axis) {
    var i, isExpansive, isExtensive, isHyperexpansive, j, k, nNonNegative, ni, nj, nk, r, sb, sr, x, _i, _j, _k, _l, _len, _len2, _ref2;
    if (axis == null) {
      axis = -1;
    }
    sb = shapeOf(b);
    ...
    for (i = 0; 0 <= ni ? i < ni : i > ni; 0 <= ni ? i++ : i--) {
      ...
      for (_j = 0, _len2 = a.length; _j < _len2; _j++) {
        ...
        if (x > 0) {
          ...
          for (_l = 0, _ref2 = -a[j] * nk; 0 <= _ref2 ? _l < _ref2 : _l > _ref2; 0 <= _ref2 ? _l++ : _l--) {
            r.push(0);
          }
          j += isExpansive;
        }
  ...
  postfixOperator('⍣', function(f) {
    return function(n) {
      if (typeof n !== 'number' || n < 0 || n !== floor(n)) {
