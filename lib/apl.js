(function() {
  var BTM, BTMLFT, BTMRGT, LFT, PI, RGT, TOP, TOPLFT, TOPRGT, abs, acos, ambivalent, arrayValueOf, asin, atan, booleanValueOf, builtins, catenate, ceil, cos, dyadic, exec, exp, floor, fnsByArity, format, format0, hpad, infixOperator, inherit, isSimple, log, main, max, min, monadic, niladic, numericValueOf, pervasive, postfixOperator, pow, prefixOperator, prod, random, reduce, repeat, shapeOf, sin, sqrt, sum, tan, vpad, withShape, _ref;
  ...
  dyadic('⌷', function(a, b) {
    ...
    r = [];
    rec = function(d, i, N) {
      var n, x, _l, _len6, _ref2, _results;
      if (d >= a.length) {
        return r.push(b[i]);
      } else {
        n = N / sb[d];
        if (isSimple(a[d])) {
          return rec(d + 1, i + a[d] * n, n);
        } else {
          _ref2 = a[d];
          _results = [];
          for (_l = 0, _len6 = _ref2.length; _l < _len6; _l++) {
            x = _ref2[_l];
            _results.push(rec(d + 1, i + x * n, n));
          }
          return _results;
        }
      }
