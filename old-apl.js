...
(function() {
  var A, ADVERB, Beta, CON, CONJUNCTION, DYA, EMB, GET, JEQ, LAM, LDC, MON, NOUN, POP, Proc, RET, SET, SPL, VEC, VERB, Z, addVocabulary, adverb, all, apl, aplError, aplify, approx, arrayEquals, bool, compileAST, complexify, compressOrReplicate, conjunction, contains, cps, deal, depthOf, domainError, enlist, eq, exec, exp, expand, extend, first, format, getAxisList, grade, indexAtSingleAxis, indexError, innerProduct, lengthError, lnΓ, match, negInt, nonceError, numApprox, numeric, outerProduct, parse, pervasive, prelude, prod, rankError, readline, reduce, repeat, repr, reversed, roll, rotate, scan, simplify, smallFactorials, squish, strideForShape, syntaxError, take, tokenDefs, tokenize, valueError, vm, vocabulary, withIdentity, Γ, _base, _ref, _ref1, _ref2,
    __slice = [].slice,
  ...
  scan = function(f, g, axis) {
    if (!(typeof g === 'undefined')) {
      throw Error("\"assert typeof g is 'undefined'\" at src/vocabulary/backslash.coffee:40");
    }
    return function(omega, alpha) {
      if (!(alpha == null)) {
        throw Error("\"assert !⍺?\" at src/vocabulary/backslash.coffee:42");
      }
  ...
  expand = function(omega, alpha, axis) {
    var a, b, data, filler, i, indices, p, shape, t63, x, _i, _len, _ref1;
    if (!omega.shape.length) {
  ...
  reduce = this.reduce = function(f, g, axis0) {
    if (!(typeof f === 'function')) {
      throw Error("\"assert typeof f is 'function'\" at src/vocabulary/slash.coffee:34");
    }
    if (!(typeof g === 'undefined')) {
      throw Error("\"assert typeof g is 'undefined'\" at src/vocabulary/slash.coffee:35");
    }
    if (!((typeof axis0 === 'undefined') || (axis0 instanceof A))) {
      throw Error("\"assert((typeof axis0 is 'undefined') or (axis0 instanceof A))\" at src/vocabulary/slash.coffee:36");
    }
    return function(omega, alpha) {
      ...
      if (omega.empty()) {
        if ((z = f.identity) != null) {
          if (!(!z.shape.length)) {
            throw Error("\"assert !⍴⍴ z\" at src/vocabulary/slash.coffee:64");
          }
  ...
  compressOrReplicate = function(omega, alpha, axis) {
    var a, b, data, filler, i, indices, n, p, shape, t163, x, _i, _j, _len, _ref2;
    if (!omega.shape.length) {
      omega = new A([omega.unwrap()]);
    }
    axis = axis ? axis.toInt(0, omega.shape.length) : omega.shape.length - 1;
  ...
  apl.ws = function(opts) {
    ...
    if (opts["in"]) {
      ctx['get_⎕'] = ctx['get_⍞'] = function() {
        ...
        s = opts["in"]();
        if (!(typeof s === 'string')) {
          throw Error("\"if opts.in then ctx['get_⎕'] = ctx['get_⍞'] = -> s = opts.in(); assert typeof s is 'string'; new A s\" at /home/nick/gh/apl/src/apl.coffee:70");
        }
        return new A(s);
      };
    }
  ...
  if (typeof module !== "undefined" && module !== null) {
    ...
    if (module === (typeof require !== "undefined" && require !== null ? require.main : void 0)) {
      (function() {
        var arg, b, f, file, k, usage, ws, _i, _len, _ref3;
