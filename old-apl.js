...
(function() {
  var ADVERB, APLArray, Beta, CONJUNCTION, Complex, DYA, EMB, GET, JEQ, LAM, LDC, MON, NOUN, POP, RET, SET, SPL, VEC, VERB, addVocabulary, adverb, aka, all, apl, aplError, aplify, approx, arrayEquals, bool, compileAST, complexify, compressOrReplicate, conjunction, contains, cps, deal, depthOf, domainError, enlist, eq, exec, exp, expand, extend, first, format, getAxisList, grade, indexError, innerProduct, lengthError, lnΓ, match, negInt, nonceError, numApprox, numeric, outerProduct, parse, pervasive, prelude, prepareForIndexing, prod, rankError, reduce, repeat, repr, roll, rotate, scan, simplify, smallFactorials, squish, strideForShape, syntaxError, take, tokenize, valueError, vm, vocabulary, withIdentity, Γ, λ, _base, _ref, _ref1,
    __slice = [].slice,
  ...
  APLArray = (function() {
    function APLArray(data, shape, stride, offset) {
      ...
      if (!(this.shape instanceof Array)) {
        throw Error("\"assert @shape instanceof Array\" at src/array.coffee:96");
      }
      if (!(this.stride instanceof Array)) {
        throw Error("\"assert @stride instanceof Array\" at src/array.coffee:97");
      }
    ...
    APLArray.prototype.map2 = function(a, f) {
      ...
      if (!(t18.length === t19.length)) {
        throw Error("\"assert shape.length is shape1.length\" at src/array.coffee:48");
      }
      for (axis = _i = 0, _ref = t18.length; 0 <= _ref ? _i < _ref : _i > _ref; axis = 0 <= _ref ? ++_i : --_i) {
        if (!(t18[t15] === t19[t15])) {
          throw Error("\"for axis in [0...shape.length] then assert shape[axis] is shape1[axis]\" at src/array.coffee:49");
        }
      }
      if (!t11.empty()) {
        t22 = t18.length - 1;
        ...
        t14 = t12.offset;
        indices = [];
        for (axis = _j = 0, _ref1 = t22; _j <= _ref1; axis = _j += 1) {
          indices.push(0);
        }
        while (true) {
          x = t16[t13];
    ...
    APLArray.prototype.toArray = function() {
      var axis, r, t23, t24, t25, t26, t27, t28, t29, t30, x, _i, _ref;
      r = [];
      t23 = this;
      if (!t23.empty()) {
        t25 = t23.data;
  ...
  strideForShape = function(shape) {
    ...
    if (!(shape instanceof Array)) {
      throw Error("\"assert shape instanceof Array\" at src/array.coffee:155");
    }
    if (shape.length === 0) {
      return [];
    }
  ...
  vm = function(_arg) {
    var a, code, env, f, frame, n, pc, size, stack, w, x, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4;
    ...
    while (true) {
      switch (code[pc++]) {
        ...
        case VEC:
          ...
          break;
        case GET:
          stack.push((_ref1 = env[code[pc++]][code[pc++]]) != null ? _ref1 : valueError());
          break;
        case SET:
          env[code[pc++]][code[pc++]] = stack[stack.length - 1];
          break;
        case MON:
          _ref2 = stack.splice(-2), w = _ref2[0], f = _ref2[1];
          if (typeof f === 'function') {
            if (w instanceof λ) {
              w = w.toFunction();
            }
          ...
          break;
        case DYA:
          _ref3 = stack.splice(-3), w = _ref3[0], f = _ref3[1], a = _ref3[2];
          if (typeof f === 'function') {
            if (w instanceof λ) {
              w = w.toFunction();
            }
        ...
        case RET:
          ...
          _ref4 = stack.splice(-4, 3), code = _ref4[0], pc = _ref4[1], env = _ref4[2];
          break;
        case POP:
          stack.pop();
          break;
  ...
  tokenize = (function() {
    ...
    return function(s) {
      ...
      b = a.toArray();
      _results = [];
      for (i = _i = 0, _ref1 = b.length, _ref2 = a.shape[1]; _ref2 > 0 ? _i < _ref1 : _i > _ref1; i = _i += _ref2) {
        _results.push({
          type: b[i],
  ...
  match = function(x, y) {
    var axis, r, t32, t33, t34, t35, t36, t37, t38, t39, t40, t41, t42, t43, t44, xi, yi, _i, _j, _k, _ref, _ref1, _ref2;
    if (x instanceof APLArray) {
      if (!(y instanceof APLArray)) {
        ...
        for (axis = _j = 0, _ref1 = t39.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; axis = 0 <= _ref1 ? ++_j : --_j) {
          if (!(t39[t36] === t40[t36])) {
            throw Error("\"for axis in [0...shape.length] then assert shape[axis] is shape1[axis]\" at src/array.coffee:49");
          }
        }
        if (!t32.empty()) {
          ...
          t35 = t33.offset;
          t44 = [];
          for (axis = _k = 0, _ref2 = t43; _k <= _ref2; axis = _k += 1) {
            t44.push(0);
          }
          while (true) {
            xi = t37[t34];
  ...
  approx = function(x, y) {
    var axis, r, t45, t46, t47, t48, t49, t50, t51, t52, t53, t54, t55, t56, t57, xi, yi, _i, _j, _k, _ref, _ref1, _ref2;
    if (x instanceof APLArray) {
      if (!(y instanceof APLArray)) {
        return false;
      } else {
        ...
        for (axis = _j = 0, _ref1 = t52.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; axis = 0 <= _ref1 ? ++_j : --_j) {
          if (!(t52[t49] === t53[t49])) {
            throw Error("\"for axis in [0...shape.length] then assert shape[axis] is shape1[axis]\" at src/array.coffee:49");
          }
        }
        if (!t45.empty()) {
          ...
          t48 = t46.offset;
          t57 = [];
          for (axis = _k = 0, _ref2 = t56; _k <= _ref2; axis = _k += 1) {
            t57.push(0);
          }
          while (true) {
            xi = t50[t47];
  ...
  getAxisList = function(axes, rank) {
    var a, i, tmp58, tmp59, tmp60, x, _i, _len, _ref, _ref1;
    ...
    if (a instanceof APLArray) {
      ...
      for (i = _i = 0, _len = a.length; _i < _len; i = ++_i) {
        x = a[i];
        if (!((tmp59 = (x)) === ~~tmp59 && ((0) <= (_ref = tmp59) && _ref < (rank)))) {
          domainError();
        }
      ...
      return a;
    } else if ((tmp60 = (a)) === ~~tmp60 && ((0) <= (_ref1 = tmp60) && _ref1 < (rank))) {
      return [a];
    } else {
      return domainError();
    }
  ...
  expand = function(omega, alpha, axis) {
    var a, b, data, filler, i, indices, p, shape, tmp61, x, _i, _len, _ref;
    if (!omega.shape.length) {
    ...
    for (_i = 0, _len = a.length; _i < _len; _i++) {
      x = a[_i];
      if (!((tmp61 = (x)) === ~~tmp61 && ((0) <= (_ref = tmp61) && _ref < (2)))) {
        domainError();
      }
      b.push(x > 0 ? i++ : null);
    }
  ...
  addVocabulary({
    ',': function(omega, alpha, axis) {
      var a, data, i, nAxes, p, pIndices, q, r, rStride, s, shape, stride, tmp62, tmp63, tmp64, tmp65, tmp66, tmp67, tmp68, _i, _j, _ref, _ref1, _ref2;
      if (alpha) {
        ...
        if ((alpha.shape.length === (_ref = omega.shape.length) && _ref === 0)) {
  ...
  depthOf = function(x) {
    var axis, r, t73, t74, t75, t76, t77, t78, t79, t80, y, _i, _ref;
    if (x instanceof APLArray) {
      ...
      if (!t73.empty()) {
        ...
        t80 = t73.offset;
        t79 = [];
        for (axis = _i = 0, _ref = t76.length; _i < _ref; axis = _i += 1) {
          t79.push(0);
        }
        while (true) {
          y = t75[t80];
  ...
  addVocabulary({
    '∪': function(omega, alpha) {
      var a, axis, data, t81, t82, t83, t84, t85, t86, t87, t88, t89, t90, t91, t92, t93, t94, t95, t96, x, _i, _j, _k, _len, _ref, _ref1, _ref2;
      if (alpha) {
        data = [];
        _ref = [alpha, omega];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (!t81.empty()) {
            ...
            t88 = t81.offset;
            t87 = [];
            for (axis = _j = 0, _ref1 = t84.length; _j < _ref1; axis = _j += 1) {
              t87.push(0);
            }
            while (true) {
              x = t83[t88];
        ...
        if (!t89.empty()) {
          ...
          t96 = t89.offset;
          t95 = [];
          for (axis = _k = 0, _ref2 = t92.length; _k < _ref2; axis = _k += 1) {
            t95.push(0);
          }
          while (true) {
            x = t91[t96];
  ...
  enlist = function(x, r) {
    var axis, t100, t101, t102, t103, t104, t105, t98, t99, y, _i, _ref, _results;
    if (x instanceof APLArray) {
      t98 = x;
      if (!t98.empty()) {
        t100 = t98.data;
        ...
        t105 = t98.offset;
        t104 = [];
        for (axis = _i = 0, _ref = t101.length; _i < _ref; axis = _i += 1) {
          t104.push(0);
        }
        _results = [];
        while (true) {
  ...
  addVocabulary({
    '⍎': function(omega, alpha) {
      var axis, c, s, t113, t114, t115, t116, t117, t118, t119, t120, _i, _ref1;
      if (alpha) {
        return nonceError();
      } else {
        s = '';
        ...
        if (!t113.empty()) {
          ...
          t120 = t113.offset;
          t119 = [];
          for (axis = _i = 0, _ref1 = t116.length; _i < _ref1; axis = _i += 1) {
            t119.push(0);
          }
          while (true) {
            c = t115[t120];
  ...
  grade = function(omega, alpha, direction) {
    var axis, h, indices, t123, t124, t125, t126, t127, t128, t129, x, _i, _j, _ref1, _ref2, _results;
    h = {};
    if (alpha) {
      if (!alpha.shape.length) {
        rankError();
    ...
    return new APLArray((function() {
      _results = [];
      for (var _j = 0, _ref2 = omega.shape[0]; 0 <= _ref2 ? _j < _ref2 : _j > _ref2; 0 <= _ref2 ? _j++ : _j--){ _results.push(_j); }
      return _results;
  ...
  addVocabulary({
    '⍳': function(omega, alpha) {
      var A, a, d, data, i, itemData, itemShape, itemStride, j, m, n, p, t138, t139, t140, t141, t142, t143, t144, t145, tmp137, u, _i, _j, _k, _l, _len, _m, _n, _o, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
      if (alpha) {
        if (alpha.shape.length !== 1) {
          rankError();
        }
        return omega.map(function(x) {
          var axis, e, indices, rank, t130, t131, t132, t133, t134, t135, t136, y, _i, _ref1;
          try {
            rank = alpha.shape;
            t130 = alpha;
            if (!t130.empty()) {
        ...
        if (!n) {
          ...
          u = n;
          for (i = _k = 0, _ref1 = a.length; _k < _ref1; i = _k += 1) {
            u /= a[i];
            p = n * i;
            for (j = _l = 0, _ref2 = a[i]; _l < _ref2; j = _l += 1) {
              itemData[p] = j;
              t138 = itemData;
              t139 = p;
              t140 = 1;
              ...
              if (t138 instanceof Array) {
                for (j = _m = _ref3 = t140, _ref4 = t141; _m < _ref4; j = _m += 1) {
            ...
            if (t142 instanceof Array) {
              for (j = _n = _ref5 = t144, _ref6 = t145; _n < _ref6; j = _n += 1) {
  ...
  addVocabulary({
    '⊂': function(omega, alpha, axes) {
      var axis, data, i, indices, p, resultAxes, shape, stride, t146, t147, t148, t149, t150, t151, unitShape, unitStride, x, _i, _j, _ref1, _ref2, _results;
      if (!(!alpha)) {
      ...
      if (!t146.empty()) {
        ...
        p = t146.offset;
        indices = [];
        for (axis = _j = 0, _ref2 = t149.length; _j < _ref2; axis = _j += 1) {
          indices.push(0);
        }
        while (true) {
          x = t148[p];
  ...
  addVocabulary({
    ...
    '⎕UCS': function(omega, alpha) {
      ...
      return omega.map(function(x) {
        var tmp154, y, _ref1;
        if ((tmp154 = (x)) === ~~tmp154 && ((0) <= (_ref1 = tmp154) && _ref1 < (0x10000))) {
  ...
  deal = function(omega, alpha) {
    var h, i, j, r, tmp156, tmp157, _i, _j, _ref1, _results;
    alpha = alpha.unwrap();
    omega = omega.unwrap();
    if (!(((tmp156 = omega) === ~~tmp156 && (0) <= tmp156) && ((tmp157 = alpha) === ~~tmp157 && ((0) <= (_ref1 = tmp157) && _ref1 < (omega + 1))))) {
      domainError();
    }
    r = (function() {
      _results = [];
  ...
  addVocabulary({
    '⍴': function(omega, alpha) {
      var a, axis, data, e, n, t159, t160, t161, t162, t163, t164, t165, t166, tmp158, x, _i, _j, _len, _ref1;
      if (alpha) {
        if (alpha.shape.length > 1) {
          rankError();
        }
        ...
        if (!n) {
          ...
          try {
            ...
            if (!t159.empty()) {
              ...
              t166 = t159.offset;
              t165 = [];
              for (axis = _j = 0, _ref1 = t162.length; _j < _ref1; axis = _j += 1) {
                t165.push(0);
              }
              while (true) {
                x = t161[t166];
  ...
  compressOrReplicate = function(omega, alpha, axis) {
    var a, b, data, filler, i, indices, n, p, shape, tmp170, x, _i, _j, _len, _ref1;
    if (!omega.shape.length) {
      omega = new APLArray([omega.unwrap()]);
    }
    axis = axis ? axis.toInt(0, omega.shape.length) : omega.shape.length - 1;
  ...
  addVocabulary({
    ...
    _substitute: function(args) {
      ...
      if (value.isSingleton()) {
        ...
        if (indexShape.length !== value.shape.length) {
          rankError();
        } else {
  ...
  prepareForIndexing = function(omega, alpha, axes) {
    var alphaItems, axis, d, i, subscriptShapes, subscripts, tmp171, tmp172, x, _i, _j, _k, _l, _len, _len1, _m, _ref1, _ref2, _ref3, _ref4, _results, _results1;
    if (!(alpha instanceof APLArray)) {
      throw Error("\"assert ⍺ instanceof APLArray\" at src/vocabulary/squish.coffee:123");
    }
    if (!(omega instanceof APLArray)) {
      throw Error("\"assert ⍵ instanceof APLArray\" at src/vocabulary/squish.coffee:124");
    }
  ...
  addVocabulary({
    '⍉': function(omega, alpha) {
      var i, n, shape, stride, tmp173, u, x, _i, _j, _len, _len1, _ref1;
      if (alpha) {
        if (alpha.shape.length > 1) {
          rankError();
        }
  ...
  exec = function(aplCode, opts) {
    var ast, code, env, k, result, v, x, _ref2, _ref3;
    if (opts == null) {
      opts = {};
    }
  ...
  repr = function(x) {
    var k, v, y, _ref2;
    if (x === null || ((_ref2 = typeof x) === 'string' || _ref2 === 'number' || _ref2 === 'boolean')) {
  ...
  compileAST = function(ast, opts) {
    var categorizeLambdas, err, i, queue, render, renderLHS, scopeNode, vars, visit, visitLHS, _i, _ref2;
    if (opts == null) {
      opts = {};
    }
    ast.scopeDepth = 0;
        ...
        switch (node[0]) {
          ...
          case 'symbol':
            ...
            if (((_ref2 = (v = vars["get_" + name])) != null ? _ref2.category : void 0) === VERB) {
              ...
              return ((_ref3 = vars[name]) != null ? _ref3.category : void 0) || valueError("Symbol '" + name + "' is referenced before assignment.", {
                file: opts.file,
                line: node.startLine,
                col: node.startCol,
                aplCode: opts.aplCode
  ...
  prelude = (function() {
    var code, env, k, nSlots, v, vars, _ref2;
    _ref2 = {"code":["LDC",new APLArray([],[0],[1],0),"SET",0,73,"POP","LAM",22,"GET",1,2,"GET",0,65,"GET",1,0,"GET",0,29,"GET",1,2,"DYA","GET",0,44,"MON","DYA","RET","GET",0,72,"GET",0,44,"DYA","SET",0,44,"POP","LAM",39,"LAM",15,"GET",2,0,"GET",1,0,"MON","GET",1,2,"GET",2,2,"DYA","RET","GET",0,72,"LAM",15,"GET",2,0,"GET",1,0,"MON","GET",1,2,"GET",2,0,"DYA","RET","DYA","RET","SET",0,74,"POP","LAM",173,"GET",1,2,"GET",0,62,"MON","GET",0,62,"MON","GET",0,16,"LDC",new APLArray([1],[],[],0),"DYA","JEQ",8,"POP","LDC",new APLArray("RANK ERROR",[10],[1],0),"GET",0,61,"MON","RET","POP","GET",1,0,"SET",1,3,"POP","GET",1,2,"LAM",123,"GET",2,0,"GET",0,62,"MON","GET",0,62,"MON","GET",0,16,"LDC",new APLArray([1],[],[],0),"DYA","JEQ",8,"POP","LDC",new APLArray("RANK ERROR",[10],[1],0),"GET",0,61,"MON","RET","POP","GET",2,0,"GET",0,13,"MON","SET",2,0,"POP","GET",1,3,"GET",0,62,"MON","GET",0,62,"MON","GET",0,15,"GET",2,0,"GET",0,62,"MON","DYA","JEQ",8,"POP","LDC",new APLArray("RANK ERROR",[10],[1],0),"GET",0,61,"MON","RET","POP","GET",1,3,"GET",0,62,"MON","GET",0,19,"GET",2,0,"DYA","GET",0,45,"GET",0,65,"MON","MON","JEQ",8,"POP","LDC",new APLArray("INDEX ERROR",[11],[1],0),"GET",0,61,"MON","RET","POP","GET",1,3,"GET",0,67,"GET",2,0,"DYA","GET",0,75,"MON","SET",1,3,"RET","GET",0,27,"MON","MON","POP","GET",1,3,"RET","GET",0,72,"LAM",229,"GET",1,0,"GET",0,62,"MON","GET",0,62,"MON","GET",0,14,"LDC",new APLArray([0],[],[],0),"DYA","JEQ",9,"POP","GET",1,0,"GET",0,70,"MON","RET","POP","GET",1,0,"GET",0,62,"MON","GET",0,3,"GET",0,65,"MON","MON","GET",0,14,"LDC",new APLArray([0],[],[],0),"DYA","JEQ",5,"POP","GET",1,0,"RET","POP","GET",1,0,"GET",0,62,"MON","SET",1,3,"POP","GET",1,0,"GET",0,13,"MON","SET",1,0,"POP","GET",1,0,"GET",0,62,"GET",0,27,"MON","MON","SET",1,4,"GET",0,79,"GET",0,27,"MON","MON","GET",0,35,"GET",0,65,"MON","MON","SET",1,5,"POP","GET",1,4,"GET",0,13,"GET",0,21,"LDC",new APLArray([1],[],[],0),"GET",0,62,"GET",1,5,"DYA","DYA","GET",0,36,"GET",0,26,"DYA","GET",0,37,"GET",0,62,"DYA","GET",0,27,"MON","MON","SET",1,4,"GET",0,35,"GET",0,65,"MON","MON","GET",0,70,"MON","SET",1,6,"POP","GET",1,0,"LAM",18,"GET",2,0,"GET",0,62,"GET",2,2,"DYA","GET",0,70,"GET",1,6,"DYA","RET","GET",0,27,"MON","GET",1,4,"DYA","GET",0,76,"GET",0,65,"MON","MON","GET",0,70,"MON","GET",0,62,"GET",1,6,"GET",0,13,"GET",1,3,"DYA","DYA","RET","DYA","SET",0,75,"POP","LAM",23,"GET",1,0,"LDC",new APLArray([0],[],[],0),"VEC",1,"LDC",new APLArray([0],[1],[1],0),"VEC",2,"GET",0,68,"GET",0,13,"DYA","GET",1,2,"DYA","RET","GET",0,72,"LAM",38,"GET",1,0,"GET",0,62,"GET",1,0,"GET",0,79,"MON","GET",1,0,"GET",0,62,"MON","GET",0,26,"LDC",new APLArray([1],[],[],0),"DYA","GET",0,3,"GET",0,65,"MON","MON","VEC",2,"DYA","RET","DYA","SET",0,76,"POP","LAM",4,"GET",1,0,"RET","SET",0,77,"POP","LAM",4,"GET",1,2,"RET","GET",0,72,"LAM",3,"LDC",new APLArray([],[0],[1],0),"RET","DYA","SET",0,78,"POP","LAM",15,"GET",1,0,"GET",0,20,"GET",1,2,"DYA","GET",0,44,"MON","RET","GET",0,72,"LAM",21,"LDC",new APLArray([1],[],[],0),"GET",0,13,"GET",1,0,"GET",0,62,"MON","DYA","GET",0,62,"GET",0,73,"DYA","RET","DYA","SET",0,79,"POP","LAM",22,"GET",1,2,"GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",1,0,"GET",0,80,"MON","DYA","RET","GET",0,72,"LAM",724,"LAM",28,"LDC",new APLArray([0.5],[],[],0),"GET",0,5,"GET",2,0,"GET",0,0,"MON","GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",2,0,"DYA","DYA","RET","SET",1,3,"POP","LAM",290,"LDC",new APLArray([1],[],[],0),"VEC",1,"LDC",new APLArray([0],[1],[1],0),"VEC",2,"GET",0,68,"GET",2,0,"GET",0,62,"MON","DYA","SET",2,3,"POP","GET",2,3,"GET",0,19,"LDC",new APLArray([1],[],[],0),"DYA","JEQ",43,"POP","GET",2,0,"LAM",35,"GET",3,0,"GET",0,13,"MON","GET",1,3,"MON","SET",3,3,"POP","GET",3,3,"GET",0,4,"GET",3,0,"DYA","GET",3,3,"GET",0,76,"MON","VEC",2,"RET","MON","RET","POP","LDC",new APLArray([2],[],[],0),"GET",0,4,"GET",2,3,"DYA","GET",0,35,"MON","SET",2,4,"POP","GET",2,0,"GET",0,70,"GET",2,4,"GET",0,13,"GET",2,0,"GET",0,62,"MON","GET",0,70,"LDC",new APLArray([1],[],[],0),"DYA","DYA","DYA","SET",2,5,"POP","GET",2,0,"GET",0,26,"GET",2,4,"GET",0,13,"LDC",new APLArray([0],[],[],0),"DYA","DYA","SET",2,6,"POP","GET",2,5,"GET",2,1,"MON","SPL",2,"SET",2,7,"POP","SET",2,8,"POP","POP","GET",2,6,"GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",2,7,"GET",0,71,"MON","GET",0,0,"MON","DYA","SET",2,9,"POP","GET",2,9,"GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",2,7,"DYA","GET",0,1,"GET",2,6,"DYA","GET",2,1,"MON","SPL",2,"SET",2,10,"POP","SET",2,11,"POP","POP","GET",2,10,"GET",0,13,"GET",2,7,"DYA","GET",2,11,"GET",0,70,"GET",2,3,"GET",0,1,"MON","GET",0,13,"LDC",new APLArray([2],[],[],0),"GET",0,4,"GET",2,3,"DYA","GET",0,34,"MON","DYA","DYA","GET",0,76,"GET",2,9,"GET",0,13,"GET",2,8,"DYA","DYA","VEC",2,"RET","SET",1,4,"POP","LAM",214,"GET",2,0,"GET",0,62,"MON","GET",0,70,"LDC",new APLArray([1],[],[],0),"DYA","SET",2,3,"GET",0,14,"LDC",new APLArray([1],[],[],0),"DYA","JEQ",9,"POP","GET",2,0,"GET",0,4,"MON","RET","POP","LDC",new APLArray([2],[],[],0),"GET",0,4,"GET",2,3,"DYA","GET",0,35,"MON","SET",2,4,"POP","GET",2,0,"GET",0,70,"GET",2,4,"GET",0,13,"GET",2,4,"DYA","DYA","GET",2,1,"MON","SET",2,5,"POP","GET",2,0,"GET",0,26,"GET",2,4,"GET",0,13,"GET",2,4,"DYA","DYA","GET",2,1,"MON","SET",2,6,"POP","GET",2,0,"GET",0,70,"GET",2,3,"GET",0,1,"GET",2,4,"DYA","GET",0,13,"GET",2,4,"DYA","DYA","SET",2,7,"POP","GET",2,6,"GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",2,7,"DYA","GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",2,5,"DYA","GET",0,1,"MON","SET",2,8,"POP","GET",2,6,"GET",0,70,"GET",2,3,"GET",0,1,"MON","GET",0,13,"LDC",new APLArray([2],[],[],0),"GET",0,4,"GET",2,3,"DYA","GET",0,34,"MON","DYA","DYA","GET",0,76,"GET",2,8,"GET",0,13,"GET",2,5,"DYA","DYA","RET","SET",1,5,"POP","GET",1,0,"GET",0,62,"MON","GET",0,62,"MON","GET",0,14,"LDC",new APLArray([0],[],[],0),"DYA","JEQ",9,"POP","GET",1,0,"GET",0,4,"MON","RET","POP","GET",1,0,"GET",0,62,"MON","GET",0,62,"MON","GET",0,14,"LDC",new APLArray([1],[],[],0),"DYA","JEQ",17,"POP","GET",1,0,"GET",0,76,"MON","GET",1,1,"MON","GET",0,13,"MON","RET","POP","GET",1,0,"GET",0,62,"MON","GET",0,62,"MON","GET",0,15,"LDC",new APLArray([2],[],[],0),"DYA","JEQ",8,"POP","LDC",new APLArray("RANK ERROR",[10],[1],0),"GET",0,61,"MON","RET","POP","GET",1,0,"GET",0,62,"MON","GET",0,19,"GET",0,65,"MON","MON","GET",0,29,"LDC",new APLArray([0],[],[],0),"DYA","JEQ",8,"POP","LDC",new APLArray("LENGTH ERROR",[12],[1],0),"GET",0,61,"MON","RET","POP","GET",1,0,"GET",1,4,"MON","SPL",2,"SET",1,6,"POP","SET",1,7,"POP","POP","GET",1,6,"GET",0,71,"MON","GET",0,0,"MON","GET",0,3,"GET",0,25,"GET",0,0,"DYA","GET",1,7,"GET",1,5,"MON","DYA","RET","DYA","SET",0,80,"POP","LAM",31,"LAM",11,"GET",2,2,"GET",1,0,"GET",2,0,"DYA","RET","GET",0,72,"LAM",11,"GET",2,0,"GET",1,0,"GET",2,0,"DYA","RET","DYA","RET","SET",0,81,"RET"],"nSlots":82,"vars":{"+":{"category":2,"slot":0,"scopeDepth":0},"-":{"category":2,"slot":1,"scopeDepth":0},"−":{"category":2,"slot":2,"scopeDepth":0},"×":{"category":2,"slot":3,"scopeDepth":0},"÷":{"category":2,"slot":4,"scopeDepth":0},"*":{"category":2,"slot":5,"scopeDepth":0},"⋆":{"category":2,"slot":6,"scopeDepth":0},"⍟":{"category":2,"slot":7,"scopeDepth":0},"|":{"category":2,"slot":8,"scopeDepth":0},"∣":{"category":2,"slot":9,"scopeDepth":0},"\\":{"category":3,"slot":10,"scopeDepth":0},"⍀":{"category":3,"slot":11,"scopeDepth":0},"○":{"category":2,"slot":12,"scopeDepth":0},",":{"category":2,"slot":13,"scopeDepth":0},"=":{"category":2,"slot":14,"scopeDepth":0},"≠":{"category":2,"slot":15,"scopeDepth":0},"<":{"category":2,"slot":16,"scopeDepth":0},">":{"category":2,"slot":17,"scopeDepth":0},"≤":{"category":2,"slot":18,"scopeDepth":0},"≥":{"category":2,"slot":19,"scopeDepth":0},"≡":{"category":2,"slot":20,"scopeDepth":0},"∘":{"category":4,"slot":21,"scopeDepth":0},"∪":{"category":2,"slot":22,"scopeDepth":0},"∩":{"category":2,"slot":23,"scopeDepth":0},"⊥":{"category":2,"slot":24,"scopeDepth":0},".":{"category":4,"slot":25,"scopeDepth":0},"↓":{"category":2,"slot":26,"scopeDepth":0},"¨":{"category":3,"slot":27,"scopeDepth":0},"⊤":{"category":2,"slot":28,"scopeDepth":0},"∊":{"category":2,"slot":29,"scopeDepth":0},"∈":{"category":2,"slot":30,"scopeDepth":0},"!":{"category":2,"slot":31,"scopeDepth":0},"⍎":{"category":2,"slot":32,"scopeDepth":0},"⍷":{"category":2,"slot":33,"scopeDepth":0},"⌊":{"category":2,"slot":34,"scopeDepth":0},"⌈":{"category":2,"slot":35,"scopeDepth":0},"_fork1":{"category":2,"slot":36,"scopeDepth":0},"_fork2":{"category":2,"slot":37,"scopeDepth":0},"⍕":{"category":2,"slot":38,"scopeDepth":0},"⍋":{"category":2,"slot":39,"scopeDepth":0},"⍒":{"category":2,"slot":40,"scopeDepth":0},"⍁":{"category":4,"slot":41,"scopeDepth":0},"⍳":{"category":2,"slot":42,"scopeDepth":0},"⊂":{"category":2,"slot":43,"scopeDepth":0},"~":{"category":2,"slot":44,"scopeDepth":0},"∨":{"category":2,"slot":45,"scopeDepth":0},"∧":{"category":2,"slot":46,"scopeDepth":0},"^":{"category":2,"slot":47,"scopeDepth":0},"⍱":{"category":2,"slot":48,"scopeDepth":0},"⍲":{"category":2,"slot":49,"scopeDepth":0},"⍣":{"category":4,"slot":50,"scopeDepth":0},"get_⎕":{"category":2,"slot":51,"scopeDepth":0},"⎕":{"category":1},"set_⎕":{"category":2,"slot":52,"scopeDepth":0},"get_⍞":{"category":2,"slot":53,"scopeDepth":0},"⍞":{"category":1},"set_⍞":{"category":2,"slot":54,"scopeDepth":0},"get_⎕IO":{"category":2,"slot":55,"scopeDepth":0},"⎕IO":{"category":1},"set_⎕IO":{"category":2,"slot":56,"scopeDepth":0},"⎕DL":{"category":2,"slot":57,"scopeDepth":0},"⎕RE":{"category":2,"slot":58,"scopeDepth":0},"⎕UCS":{"category":2,"slot":59,"scopeDepth":0},"?":{"category":2,"slot":60,"scopeDepth":0},"↗":{"category":2,"slot":61,"scopeDepth":0},"⍴":{"category":2,"slot":62,"scopeDepth":0},"⌽":{"category":2,"slot":63,"scopeDepth":0},"⊖":{"category":2,"slot":64,"scopeDepth":0},"/":{"category":3,"slot":65,"scopeDepth":0},"⌿":{"category":3,"slot":66,"scopeDepth":0},"⌷":{"category":2,"slot":67,"scopeDepth":0},"_index":{"category":2,"slot":68,"scopeDepth":0},"_substitute":{"category":2,"slot":69,"scopeDepth":0},"↑":{"category":2,"slot":70,"scopeDepth":0},"⍉":{"category":2,"slot":71,"scopeDepth":0},"⍠":{"category":4,"slot":72,"scopeDepth":0},"⍬":{"scopeDepth":0,"slot":73,"category":1},"_hook":{"scopeDepth":0,"slot":74,"category":4},"⊃":{"scopeDepth":0,"slot":75,"category":2},"⍪":{"scopeDepth":0,"slot":76,"category":2},"⊢":{"scopeDepth":0,"slot":77,"category":2},"⊣":{"scopeDepth":0,"slot":78,"category":2},"≢":{"scopeDepth":0,"slot":79,"category":2},"⌹":{"scopeDepth":0,"slot":80,"category":2},"⍨":{"scopeDepth":0,"slot":81,"category":3}}}, code = _ref2.code, nSlots = _ref2.nSlots, vars = _ref2.vars;
    env = [[]];
    for (k in vars) {
      v = vars[k];
      env[0][v.slot] = vocabulary[k];
  ...
  apl.ws = function(opts) {
    ...
    if (opts["in"]) {
      ctx['get_⎕'] = ctx['get_⍞'] = function() {
        ...
        s = opts["in"]();
        if (!(typeof s === 'string')) {
          throw Error("\"if opts.in then ctx['get_⎕'] = ctx['get_⍞'] = -> s = opts.in(); assert typeof s is 'string'; new APLArray s\" at /home/nick/gh/apl/src/apl.coffee:61");
        }
  ...
  if (typeof module !== "undefined" && module !== null) {
    ...
    if (module === (typeof require !== "undefined" && require !== null ? require.main : void 0)) {
      (function() {
        var arg, b, file, k, rl, usage, ws, _i, _len, _ref2;
        ...
        if (file != null) {
          ...
          process.stdout.write("ngn apl 2013-12-01\n");
