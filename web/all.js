...
var APLArray, DomainError, LengthError, isInt, prod, strideForShape, _ref, _ref1;



_ref = require('./helpers'), prod = _ref.prod, isInt = _ref.isInt;
...
this.APLArray = APLArray = (function() {
  function APLArray(data, shape, stride, offset) {
    ...
    if (!(this.data.length === 0 || isInt(this.offset, 0, this.data.length))) {
    ...
    if (this.data.length) {
      ...
      for (i = _j = 0, _len1 = _ref3.length; _j < _len1; i = ++_j) {
        x = _ref3[i];
        if (!(isInt(x, -this.data.length, this.data.length + 1))) {
...
this.repeat = repeat = function(a, n) {
  ...
  if (!(typeof a === 'string' || a instanceof Array)) {
    throw Error("\"assert typeof a is 'string' or a instanceof Array\" at src/helpers.coffee:7");
  }
  if (!(isInt(n, 0))) {
    throw Error("\"assert isInt n, 0\" at src/helpers.coffee:8");
  }
  if (!n) {
    return a.slice(0, 0);
  }
...
var APLArray, DomainError, LengthError, NonceError, RankError, adverb, expand, isInt, repeat, scan, _ref, _ref1;



APLArray = require('../array').APLArray;

_ref = require('../helpers'), isInt = _ref.isInt, repeat = _ref.repeat;

_ref1 = require('../errors'), RankError = _ref1.RankError, NonceError = _ref1.NonceError, DomainError = _ref1.DomainError, LengthError = _ref1.LengthError;
...
scan = function(f, g, axis) {
  ...
  return function(omega, alpha) {
    ...
    return omega.map(function(x, indices) {
      var a, index, j, p, y, _i, _j, _len, _ref2;
      p = omega.offset;
      ...
      for (j = _j = 0, _ref2 = indices[axis]; _j < _ref2; j = _j += 1) {
        p -= omega.stride[axis];
        y = omega.data[p];
...
expand = function(omega, alpha, axis) {
  var a, b, data, filler, i, indices, p, shape, x, _i, _len;
  if (omega.shape.length === 0) {
...
var APLArray, DomainError, LengthError, RankError, catenate, isInt, prod, repeat, _ref, _ref1;
...
_ref1 = require('../helpers'), prod = _ref1.prod, repeat = _ref1.repeat, isInt = _ref1.isInt;
...
this.vocabulary = {
  '↓': function(omega, alpha, axis) {
    var a, array, data, i, ishape, obound, offset, oshape, shape, x, _i, _j, _k, _len, _len1;
    if (alpha) {
      if (alpha.shape.length > 1) {
        throw RankError();
      }
...
var APLArray, Beta, DomainError, isInt, lnΓ, negInt, pervasive, prod, real, smallFactorials, withIdentity, Γ, _ref, _ref1;



_ref = require('./vhelpers'), pervasive = _ref.pervasive, real = _ref.real, withIdentity = _ref.withIdentity;

_ref1 = require('../helpers'), isInt = _ref1.isInt, prod = _ref1.prod;
...
var APLArray, Complex, DomainError, RankError, aka, bool, isInt, match, negate, pervasive, real, withIdentity, _ref, _ref1;
...
var APLArray, conjunction, isInt;
...
this.vocabulary = {
  '⍣': conjunction(function(g, f) {
    ...
    if (f instanceof APLArray && typeof g === 'function') {
      ...
      if (!(typeof f === 'function')) {
        throw Error("\"assert typeof f is 'function'\" at src/vocabulary/poweroperator.coffee:20");
      }
    }
    if (typeof g === 'function') {
      return function(omega, alpha) {
...
_ref1 = require('../helpers'), prod = _ref1.prod, isInt = _ref1.isInt, repeat = _ref1.repeat;
...
var APLArray, DomainError, IndexError, LengthError, isInt, prod, repeat, rotate, _ref, _ref1;
...
var APLArray, DomainError, LengthError, RankError, adverb, compressOrReplicate, isInt, reduce, repeat, _ref, _ref1;
...
_ref1 = require('../helpers'), repeat = _ref1.repeat, isInt = _ref1.isInt;
...
reduce = this.reduce = function(f, g, axis0) {
  ...
  return function(omega, alpha) {
    var a, axis, data, i, indices, isBackwards, isMonadic, isNWise, n, p, rShape, shape, x, y, z, _i, _j, _ref2, _ref3;
    if (omega.shape.length === 0) {
      omega = new APLArray([omega.unwrap()]);
    }
    ...
    if (omega.empty()) {
      if ((z = (_ref2 = f.aplMetaInfo) != null ? _ref2.identity : void 0) != null) {
        if (!(z.shape.length === 0)) {
    ...
    while (true) {
      if (isBackwards) {
        ...
        x = x instanceof APLArray ? x : APLArray.scalar(x);
        for (i = _j = _ref3 = n - 2; _j >= 0; i = _j += -1) {
          y = omega.data[p + i * omega.stride[axis]];
          y = y instanceof APLArray ? y : APLArray.scalar(y);
          x = f(x, y);
        }
...
var APLArray, DomainError, IndexError, LengthError, RankError, isInt, prepareForIndexing, prod, repeat, squish, _ref, _ref1;
...
prepareForIndexing = function(omega, alpha, axes) {
  var alphaItems, axis, d, i, subscriptShapes, subscripts, x, _i, _j, _k, _l, _len, _len1, _m, _ref2, _ref3, _ref4, _ref5, _results, _results1;
...
var APLArray, DomainError, LengthError, RankError, isInt, _ref;
...
this.vocabulary = {
  '⍉': function(omega, alpha) {
    var i, n, shape, stride, x, _i, _len, _ref1;
    if (alpha) {
      if (alpha.shape.length > 1) {
        throw RankError();
      }
...
var APLArray, Complex, DomainError, LengthError, RankError, SyntaxError, approx, complexify, eps, isInt, match, meta, multiplicitySymbol, numApprox, _ref, _ref1,
  __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };
...
this.pervasive = function(_arg) {
  ...
  return F = function(omega, alpha) {
    if (!(omega instanceof APLArray)) {
      throw Error("\"assert omega instanceof APLArray\" at src/vocabulary/vhelpers.coffee:54");
    }
    if (!(alpha instanceof APLArray || typeof alpha === 'undefined')) {
      throw Error("\"assert alpha instanceof APLArray or typeof alpha is 'undefined'\" at src/vocabulary/vhelpers.coffee:55");
    }
...
this.getAxisList = function(axes, rank) {
  var a, i, x, _i, _len;
  if (!(isInt(rank, 0))) {
    throw Error("\"assert isInt rank, 0\" at src/vocabulary/vhelpers.coffee:125");
  }
  if (typeof axes === 'undefined') {
    return [];
  }
  if (!(axes instanceof APLArray)) {
    throw Error("\"assert axes instanceof APLArray\" at src/vocabulary/vhelpers.coffee:127");
  }
...
meta = function(f, name, value) {
  if (!(typeof f === 'function')) {
    throw Error("\"assert typeof f is 'function'\" at src/vocabulary/vhelpers.coffee:145");
  }
  if (!(typeof name === 'string')) {
    throw Error("\"assert typeof name is 'string'\" at src/vocabulary/vhelpers.coffee:146");
  }
...
this.aka = function(aliases, f) {
  if (typeof aliases === 'string') {
    ...
    if (!(aliases instanceof Array)) {
      throw Error("\"assert aliases instanceof Array\" at src/vocabulary/vhelpers.coffee:162");
    }
  }
...
jQuery(function($) {
  var aplKeys, asciiKeys, code, combos, execute, hSymbolDefs, hashParams, i, k, layout, name, nameValue, tipsyOpts, v, value, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _ref3;
  hashParams = {};
  if (location.hash) {
