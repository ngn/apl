(function() {
  var BTM, BTMLFT, BTMRGT, LFT, PI, RGT, TOP, TOPLFT, TOPRGT, abs, ambivalent, arrayValueOf, booleanValueOf, builtins, catenate, ceil, dyadic, exec, exp, floor, fnsByArity, format, format0, hpad, infixOperator, inherit, isSimple, main, max, min, monadic, niladic, numericValueOf, pervasive, postfixOperator, pow, prefixOperator, prod, random, reduce, repeat, shapeOf, sum, vpad, withShape, _ref;
  ...
  min = Math.min, max = Math.max, abs = Math.abs, floor = Math.floor, ceil = Math.ceil, random = Math.random, exp = Math.exp, pow = Math.pow, PI = Math.PI;
  ...
  fnsByArity = [{}, {}];
  ambivalent = function(symbol) {
    var f, _ref2;
    f = (_ref2 = builtins[symbol]) != null ? _ref2 : builtins[symbol] = function(a, b) {
      if (b != null) {
        return fnsByArity[1][symbol](a, b);
      } else {
        return fnsByArity[0][symbol](a);
      }
    };
    f.name = symbol;
    f.aplName = symbol;
  ...
  monadic('?', pervasive(function(x) {
    return ceil(random() * max(0, floor(numericValueOf(x))));
  }));
  dyadic('?', function(x, y) {
    ...
    for (_j = 0; 0 <= x ? _j < x : _j > x; 0 <= x ? _j++ : _j--) {
      _results2.push(available.splice(floor(available.length * random()), 1));
    }
    return _results2;
  });
  monadic('*', pervasive(function(x) {
  ...
  postfixOperator('⌿', function(f) {
    return function(a, b) {
      return reduce(f, a, b, 0);
    };
  });
  reduce = function(f, a, b, axis) {
  ...
  exec = exports.exec = function(ast, ctx) {
    var a, f, i, j, name, r, value, x, y, _i, _len, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
    if (!(ctx != null)) {
            ...
            if (typeof a[i] === 'function') {
              if (typeof a[i + 1] === 'function' && a[i + 1].isInfixOperator) {
                if (typeof a[i + 2] === 'function') {
                  [].splice.apply(a, [i, i + 2 - i + 1].concat(_ref5 = a[i + 1](a[i + 2], a[i]))), _ref5;
                  continue;
                }
              }
            }
