(function() {
  var ambivalent, arrayValueOf, assert, booleanValueOf, builtins, catenate, compressOrReplicate, cps, cpsify, depthOf, dyadic, expand, grade, infixOperator, isSimple, match, monadic, named, numericValueOf, outerProduct, overloadable, pervasive, postfixOperator, prefixOperator, prod, prototypeOf, reduce, repeat, reverse, scan, shapeOf, sum, withPrototype, withPrototypeCopiedFrom, withShape, _ref,
  ...
  numericValueOf = function(x) {
    if (x.length != null) {
      if (x.length !== 1) throw Error('Numeric scalar or singleton expected');
      x = x[0];
    }
    if (typeof x !== 'number') throw Error('Numeric scalar or singleton expected');
    return x;
  };

  booleanValueOf = function(x) {
    x = numericValueOf(x);
    if (x !== 0 && x !== 1) throw Error('Boolean values must be either 0 or 1');
    return x;
  };
      ...
      if (b != null) {
        if (isSimple(a) && isSimple(b)) {
          ...
          for (i = 0, _ref2 = Math.min(sa.length, sb.length); 0 <= _ref2 ? i < _ref2 : i > _ref2; 0 <= _ref2 ? i++ : i--) {
            if (sa[i] !== sb[i]) throw Error('Length error');
          }
  ...
  dyadic(overloadable(named('⍳', function(a, b) {
    var i, pos, x, y, _i, _len, _len2, _results;
    if (isSimple(a)) {
      a = [a];
  ...
  dyadic(overloadable(named('≡', match = function(a, b) {
    ...
    if (isSimple(a) && isSimple(b)) return +(a === b);
    if (isSimple(a) !== isSimple(b)) return 0;
    sa = shapeOf(a);
    sb = shapeOf(b);
    ...
    for (i = 0, _ref3 = a.length; 0 <= _ref3 ? i < _ref3 : i > _ref3; 0 <= _ref3 ? i++ : i--) {
      if (!match(a[i], b[i])) return 0;
    }
    if (a.length) return 1;
    if (!((a.aplPrototype != null) || (b.aplPrototype != null))) return 1;
  ...
  dyadic(overloadable(named('⍷', function(a, b) {
    var i, indices, r, rec, rec2, sa, sb, _ref2;
    sa = shapeOf(a);
    sb = shapeOf(b);
    if (isSimple(b)) return isSimple(a) && match(a, b);
    if (isSimple(a)) a = [a];
    r = withShape(sb, (function() {
      var _i, _ref2, _results;
      _results = [];
      for (_i = 0, _ref2 = b.length; 0 <= _ref2 ? _i < _ref2 : _i > _ref2; 0 <= _ref2 ? _i++ : _i--) {
        _results.push(0);
      }
      return _results;
    })());
    if (sa.length > sb.length) return r;
    while (sa.length < sb.length) {
      sa.unshift(1);
    }
    for (i = 0, _ref2 = sb.length; 0 <= _ref2 ? i < _ref2 : i > _ref2; 0 <= _ref2 ? i++ : i--) {
      if (sa[i] > sb[i]) return r;
    }
    ...
    rec = function(d, ir) {
      ...
      if (d < sb.length) {
        _results = [];
        for (i = 0, _ref3 = sb[d] - sa[d] + 1; 0 <= _ref3 ? i < _ref3 : i > _ref3; 0 <= _ref3 ? i++ : i--) {
          indices[d] = i;
    ...
    rec2 = function(d, ia, ib) {
      var i, _ref3;
      if (d < sb.length) {
        for (i = 0, _ref3 = sa[d]; 0 <= _ref3 ? i < _ref3 : i > _ref3; 0 <= _ref3 ? i++ : i--) {
          if (!rec2(d + 1, ia * sa[d] + i, ib * sb[d] + indices[d] + i)) return 0;
        }
        return 1;
      } else {
  ...
  dyadic(overloadable(named('∼', function(a, b) {
    var excluded, r, x, y, _i, _j, _len, _len2;
    if (isSimple(a)) {
      a = [a];
    } else if (shapeOf(a).length > 1) {
      throw Error('Left argument to ∼ must be of rank no more than 1.');
  ...
  dyadic(pervasive(overloadable(named('∨', function(x, y) {
    ...
    if (x === 0 && y === 0) return 0;
    if (x < y) _ref2 = [y, x], x = _ref2[0], y = _ref2[1];
    while (y) {
  ...
  monadic(overloadable(named('⌽', reverse = function(a, _, axis) {
    var i, j, k, ni, nj, nk, r, sa, _ref2;
    if (axis == null) axis = -1;
    sa = shapeOf(a);
    if (sa.length === 0) return a;
    if (axis < 0) axis += sa.length;
    if (!((0 <= axis && axis < sa.length))) throw Error('Axis out of bounds');
    ni = prod(sa.slice(0, axis));
    nj = sa[axis];
    nk = prod(sa.slice(axis + 1));
    r = [];
    for (i = 0; 0 <= ni ? i < ni : i > ni; 0 <= ni ? i++ : i--) {
      for (j = _ref2 = nj - 1; j >= 0; j += -1) {
        for (k = 0; 0 <= nk ? k < nk : k > nk; 0 <= nk ? k++ : k--) {
  ...
  dyadic(overloadable(named('⌽', function(a, b) {
    var i, n, sb;
    a = numericValueOf(a);
    if (a === 0 || isSimple(b) || (b.length <= 1)) return b;
    sb = shapeOf(b);
    n = sb[sb.length - 1];
    a %= n;
    if (a < 0) a += n;
  ...
  dyadic(overloadable(named('↓', function(a, b) {
    ...
    lims = (function() {
      ...
      for (i = 0, _ref4 = a.length; 0 <= _ref4 ? i < _ref4 : i > _ref4; 0 <= _ref4 ? i++ : i--) {
        if (a[i] >= 0) {
          _results.push([Math.min(a[i], sb[i]), sb[i]]);
        } else {
          _results.push([0, Math.max(0, sb[i] + a[i])]);
        }
      }
      return _results;
    })();
  ...
  monadic(overloadable(named('⊃', function(a) {
    ...
    if (isSimple(a)) return a;
    sa = shapeOf(a);
    if (sa.length === 0) return a[0];
  ...
  dyadic(overloadable(named('⌷', function(a, b) {
    ...
    for (d = 0, _len3 = a.length; d < _len3; d++) {
      ...
      for (_j = 0, _len4 = x.length; _j < _len4; _j++) {
        y = x[_j];
        if (!((0 <= y && y < sb[d]))) throw Error('Index out of bounds');
      }
    }
    sr = [];
  ...
  monadic(overloadable(named('⊤', function(a, b) {
    var i, j, k, m, n, r, sa, sb, x, y, _ref2, _ref3;
    sa = shapeOf(a);
    sb = shapeOf(b);
    if (isSimple(a)) a = [a];
    if (isSimple(b)) b = [b];
    ...
    for (i = 0; 0 <= m ? i < m : i > m; 0 <= m ? i++ : i--) {
      for (j = 0, _ref2 = b.length; 0 <= _ref2 ? j < _ref2 : j > _ref2; 0 <= _ref2 ? j++ : j--) {
        y = b[j];
        for (k = _ref3 = n - 1; _ref3 <= 0 ? k <= 0 : k >= 0; _ref3 <= 0 ? k++ : k--) {
          x = a[k * m + i];
          if (x === 0) {
  ...
  monadic(overloadable(named('⊥', function(a, b) {
    var firstDimB, i, j, k, lastDimA, r, sa, sb, x, y, z, _ref2, _ref3, _ref4;
    sa = shapeOf(a);
    sb = shapeOf(b);
    ...
    if (isSimple(b)) b = [b];
    r = [];
    for (i = 0, _ref2 = a.length / lastDimA; 0 <= _ref2 ? i < _ref2 : i > _ref2; 0 <= _ref2 ? i++ : i--) {
      for (j = 0, _ref3 = b.length / firstDimB; 0 <= _ref3 ? j < _ref3 : j > _ref3; 0 <= _ref3 ? j++ : j--) {
        x = a.slice(i * lastDimA, ((i + 1) * lastDimA));
        y = (function() {
          var _results;
          _results = [];
          for (k = 0; 0 <= firstDimB ? k < firstDimB : k > firstDimB; 0 <= firstDimB ? k++ : k--) {
        ...
        if (x.length === 1) {
          x = (function() {
            ...
            _results = [];
            for (_i = 0, _ref4 = y.length; 0 <= _ref4 ? _i < _ref4 : _i > _ref4; 0 <= _ref4 ? _i++ : _i--) {
              _results.push(x[0]);
            }
            return _results;
          })();
        ...
        if (y.length === 1) {
          y = (function() {
            var _i, _ref4, _results;
            _results = [];
            for (_i = 0, _ref4 = x.length; 0 <= _ref4 ? _i < _ref4 : _i > _ref4; 0 <= _ref4 ? _i++ : _i--) {
              _results.push(y[0]);
            }
            return _results;
          })();
        ...
        z = y[0];
        for (k = 1, _ref4 = y.length; 1 <= _ref4 ? k < _ref4 : k > _ref4; 1 <= _ref4 ? k++ : k--) {
          z = z * x[k] + y[k];
        }
        r.push(z);
      }
      ...
      r = (function() {
        ...
        if (isBackwards) {
          ...
          for (i = 0, _ref3 = n - a + 1; 0 <= _ref3 ? i < _ref3 : i > _ref3; 0 <= _ref3 ? i++ : i--) {
            x = items[i + a - 1];
            for (j = _ref4 = i + a - 2, _ref5 = i - 1; j > _ref5; j += -1) {
              x = f(x, items[j]);
            }
            _results.push(x);
          }
          ...
          for (i = 0, _ref6 = n - a + 1; 0 <= _ref6 ? i < _ref6 : i > _ref6; 0 <= _ref6 ? i++ : i--) {
            x = items[i];
            for (j = _ref7 = i + 1, _ref8 = i + a; j < _ref8; j += 1) {
              x = f(x, items[j]);
            }
            _results2.push(x);
          }
  ...
  compressOrReplicate = function(a, b, axis) {
    ...
    sb = shapeOf(b);
    if (axis < 0) axis += sb.length;
    if (!((0 <= axis && axis < sb.length))) throw Error('Axis out of bounds');
    sr = sb.slice(0);
    sr[axis] = 0;
  ...
  scan = function(f, _, axis) {
    if (axis == null) axis = -1;
    return function(a, b) {
      var i, ijk, j, k, ni, nj, nk, r, sa, x;
      if (b != null) throw Error('Scan can only be applied monadically.');
      sa = shapeOf(a);
      if (sa.length === 0) return a;
      if (axis < 0) axis += sa.length;
      r = Array(a.length);
      ...
      for (i = 0; 0 <= ni ? i < ni : i > ni; 0 <= ni ? i++ : i--) {
        for (k = 0; 0 <= nk ? k < nk : k > nk; 0 <= nk ? k++ : k--) {
          ...
          for (j = 1; 1 <= nj ? j < nj : j > nj; 1 <= nj ? j++ : j--) {
            ijk = k + nk * (j + nj * i);
            x = r[ijk] = f(x, a[ijk]);
          }
        }
